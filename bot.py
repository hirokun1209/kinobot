...
# ========= 領域比率（フル画像基準） =========
HEAD_TOP_RATIO     = float(os.getenv("HEAD_TOP_RATIO",     "0.00"))
HEAD_BOTTOM_RATIO  = float(os.getenv("HEAD_BOTTOM_RATIO",  "0.25"))
HEAD_RIGHT_RATIO   = float(os.getenv("HEAD_RIGHT_RATIO",   "1.00"))

CLOCK_LEFT_RATIO   = float(os.getenv("CLOCK_LEFT_RATIO",   "0.78"))
CLOCK_TOP_RATIO    = float(os.getenv("CLOCK_TOP_RATIO",    "0.00"))
CLOCK_RIGHT_RATIO  = float(os.getenv("CLOCK_RIGHT_RATIO",  "1.00"))
# 下側を短くしたい時は環境変数で。例: CLOCK_BOTTOM_RATIO=0.11
CLOCK_BOTTOM_RATIO = float(os.getenv("CLOCK_BOTTOM_RATIO", "0.12"))

CENTER_LEFT_RATIO  = float(os.getenv("CENTER_LEFT_RATIO",  "0.00"))
CENTER_TOP_RATIO   = float(os.getenv("CENTER_TOP_RATIO",   "0.25"))  # 中央の上端を詰めたいならここを上げる
CENTER_RIGHT_RATIO = float(os.getenv("CENTER_RIGHT_RATIO", "1.00"))
CENTER_BOTTOM_RATIO= float(os.getenv("CENTER_BOTTOM_RATIO","0.78"))

CEASE_TOP_RATIO    = float(os.getenv("CEASE_TOP_RATIO",    "0.78"))
CEASE_BOTTOM_RATIO = float(os.getenv("CEASE_BOTTOM_RATIO", "1.00"))

# ========= 合成画像の「上部ヘッダ帯」を composite から読むときの比率 =========
...
# ========= 合成後にさらにトリミングする（ノーオペ既定） =========
COMP_TRIM_LEFT_RATIO   = float(os.getenv("COMP_TRIM_LEFT_RATIO",   "0"))
COMP_TRIM_TOP_RATIO    = float(os.getenv("COMP_TRIM_TOP_RATIO",    "0"))
COMP_TRIM_RIGHT_RATIO  = float(os.getenv("COMP_TRIM_RIGHT_RATIO",  "1"))
COMP_TRIM_BOTTOM_RATIO = float(os.getenv("COMP_TRIM_BOTTOM_RATIO", "1"))

# ========= フラグ =========
USE_COMPOSITE_FOR_HEADER = os.getenv("USE_COMPOSITE_FOR_HEADER", "1") == "1"

# ========= 古い名前互換（コード内で参照していた場合の保険） =========
HEAD_INNER_TOP     = HEAD_TOP_RATIO
HEAD_INNER_BOTTOM  = HEAD_BOTTOM_RATIO
HEAD_INNER_RIGHT   = HEAD_RIGHT_RATIO
...
def _mark_regions_on_full(full_bgr: np.ndarray):
    ...
    # 中央の右端は時計の左端を超えないようにクランプ（右パネル混入を防ぐ）
    ...
    _draw_box(dbg, clock_rect,  (0, 255, 255), 2, "CLOCK")   # ← コンマ抜け修正
...
CENTER_TOP_PAD_RATIO    = float(os.getenv("CENTER_TOP_PAD_RATIO", "0.15"))  # ← 上端を下へ 2%
...
def percent_crop(bgr, left=0.0, top=0.0, right=1.0, bottom=1.0):
    """画像を比率でクロップ（right/bottom は残す端の割合）。最小1px確保。"""
    if bgr is None:
        return None
    h, w = bgr.shape[:2]
    x1 = max(0, min(w, int(round(w * float(left)))))
    y1 = max(0, min(h, int(round(h * float(top)))))
    x2 = max(0, min(w, int(round(w * float(right)))))
    y2 = max(0, min(h, int(round(h * float(bottom)))))
...
async def on_message(message):
    ...
    if message.content.strip() == "!oaiocr":
        ...
        head_src_bgr = full_bgr  # 既定は原寸のヘッダ帯
        if USE_COMPOSITE_FOR_HEADER:
            comp_png = ((j.get("_echo") or {}).get("composite_png") or None)
            if comp_png:
                comp_bgr2 = _bgr_from_png_bytes(comp_png)
            else:
                comp_bgr2, _ = compose_center_with_clock_and_cease(full_bgr)
            if comp_bgr2 is not None:
                Hc, Wc = comp_bgr2.shape[:2]
                y1c = int(Hc * COMP_HEAD_TOP);  y2c = int(Hc * COMP_HEAD_BOTTOM)
                x1c = 0;                        x2c = int(Wc * COMP_HEAD_RIGHT)
                head_src_bgr = comp_bgr2[y1c:y2c, x1c:x2c]
